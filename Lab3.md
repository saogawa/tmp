...
```jsx
configure system name lab3-sr1
```
...
configure router "Base" interface "To-Peering-LAN" ipv4 primary address 192.168.3.1
configure router "Base" interface "To-Peering-LAN" ipv4 primary prefix-length 24
configure router "Base" interface "To-Peering-LAN" ipv6 address 2000:1:3::1 prefix-length 64
...
configure system time ntp server 192.168.3.254 router-instance "Base" prefer true
```
...
```jsx
    ...
    /configure router "Base" bgp neighbor "192.168.3.3" admin-state enable
    /configure router "Base" bgp neighbor "192.168.3.3" group "eBGP-Peering"
    /configure router "Base" bgp neighbor "2000:1:3::3" group "eBGP-Peering"
    /configure router "Base" bgp neighbor "2000:1:3::3" local-address 2000:1:2::1
...
configure cflowd collector 192.168.3.254 port 5000 description "Neighbor collector"
configure cflowd collector 192.168.3.254 port 5000 autonomous-system-type peer
configure cflowd collector 192.168.3.254 port 5000 version 8
configure cflowd collector 192.168.3.254 port 5000 aggregation protocol-port true
configure cflowd collector 192.168.3.254 port 5000 aggregation source-destination-prefix true
configure cflowd collector 192.168.3.254 port 2000 description "v9collector"
configure cflowd collector 192.168.3.254 port 2000 template-set mpls-ip
configure cflowd collector 192.168.3.254 port 2000 version 9
...
```jsx
configure router "Base" origin-validation rpki-session 192.168.3.5 admin-state enable
configure router "Base" origin-validation rpki-session 192.168.3.5 local-address 192.168.3.1
configure router "Base" origin-validation rpki-session 192.168.3.5 port 8282
...
configure router "Base" bgp group "ddos" peer-as 65000
configure router "Base" bgp group "ddos" local-as as-number 65000
configure router "Base" bgp neighbor "192.168.3.4" group "ddos"
configure router "Base" bgp neighbor "192.168.3.4" type internal
configure router "Base" bgp neighbor "192.168.3.4" peer-as 65000
configure router "Base" bgp neighbor "192.168.3.4" family ipv4 true
configure router "Base" bgp neighbor "192.168.3.4" family ipv6 false
configure router "Base" bgp neighbor "192.168.3.4" family flow-ipv4 true
configure router "Base" bgp neighbor "192.168.3.4" family flow-ipv6 false
configure router "Base" bgp neighbor "192.168.3.4" family ipv4 ipv6 flow-ipv4 flow-ipv6 true
...
/configure log snmp-trap-group "90" trap-target "1" address 192.168.3.254
...
/configure log syslog "syslog_server" address 192.168.3.254
